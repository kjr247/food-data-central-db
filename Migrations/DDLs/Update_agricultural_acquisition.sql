...
BEGIN TRY  
	...
	set @tableName = 'agricultural_acquisition';
		-- CHECKSUM
	...
	TRUNCATE TABLE agricultural_acquisition;
	DROP TABLE IF EXISTS #tmp; 
	-- Mapping
	create table #tmp( fdc_id nvarchar(max) NOT NULL, acquisition_date nvarchar(max) NULL, market_class nvarchar(max) NULL, treatment nvarchar(max) NULL, state nvarchar(max) NULL)

	bulk insert #tmp
	From 'C:\Users\kjr24\Downloads\FoodData_Central_csv_2024-04-18\FoodData_Central_csv_2024-04-18\agricultural_samples.csv' -- UPDATE FILE name, (!sometimes the file names are different <crosses eyes>)
	WITH
		(
			CODEPAGE = '65001'
			,FIRSTROW = 2
	...
	-- DDL
	insert into agricultural_acquisition(fdc_id, acquisition_date, market_class, treatment, state) -- UPDATE file name, and columns
		select 
			CONVERT(INT, REPLACE(t.fdc_id,'"', '')) as fdc_id
			, CONVERT(datetime2(0), REPLACE(t.acquisition_date,'"', ''))
			,REPLACE(t.market_class, '"', '')
			,REPLACE(t.treatment, '"', '')
			,REPLACE(t.state, '"', '')
		from #tmp t
		WHERE NOT EXISTS ( -- skip duplicates
			SELECT 1 FROM agricultural_acquisition AS d -- UPDATE NAME
			WHERE d.fdc_id = CONVERT(INT, REPLACE(t.fdc_id,'"', ''))
		)
