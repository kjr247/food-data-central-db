...
BEGIN TRY  
	set @tableName = 'food_attribute_type';
		-- CHECKSUM
	...
	TRUNCATE TABLE food_attribute_type;
	DROP TABLE IF EXISTS #tmp; 
	-- Mapping
	CREATE TABLE #tmp([id] [nvarchar](max) NOT NULL,	[name] [nvarchar](255) NULL,	[description] [nvarchar](255) NULL)

	bulk insert #tmp
	From 'C:\Users\kjr24\Downloads\FoodData_Central_csv_2024-04-18\FoodData_Central_csv_2024-04-18\food_attribute_type.csv' -- UPDATE FILE name, (!sometimes the file names are different <crosses eyes>)
	WITH
		(
			CODEPAGE = '65001'
			,FIRSTROW = 2
	...
	-- DDL
	insert into food_attribute_type(id,	name,	description) -- UPDATE file name, and columns
		select 
		...
		WHERE NOT EXISTS ( -- skip duplicates
			SELECT 1 FROM food_attribute_type AS d -- UPDATE NAME
