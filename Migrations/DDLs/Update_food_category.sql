...
BEGIN TRY  
	...
	bulk insert #tmp
	From 'C:\Users\kjr24\Downloads\FoodData_Central_csv_2024-04-18\FoodData_Central_csv_2024-04-18\food_category.csv' -- UPDATE FILE name, (!sometimes the file names are different <crosses eyes>)
	WITH
		(
			CODEPAGE = '65001'
			,FIRSTROW = 2
	...
	-- DDL
	insert into food_category(id, code, description) -- UPDATE file name, and columns
		select 
		...
		WHERE NOT EXISTS ( -- skip duplicates
			SELECT 1 FROM food_category AS d -- UPDATE NAME
