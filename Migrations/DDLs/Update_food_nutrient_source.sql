...
BEGIN TRY  
	set @tableName = 'food_nutrient_source';
	...
	TRUNCATE TABLE food_nutrient_source;

	DROP TABLE IF EXISTS #tmp; 
	-- Mapping
	create table #tmp([id] [nvarchar](max) NOT NULL,[code] [nvarchar](max) NULL,[description] [nvarchar](max) NULL)

	bulk insert #tmp
	From 'C:\Users\kjr24\Downloads\FoodData_Central_csv_2024-04-18\FoodData_Central_csv_2024-04-18\food_nutrient_source.csv' -- UPDATE FILE name, (!sometimes the file names are different <crosses eyes>)
	WITH
		(
			CODEPAGE = '65001'
			,FIRSTROW = 2
	...
	-- DDL
	insert into food_nutrient_source(id, code, description) -- UPDATE file name, and columns
		select 
			CONVERT(INT, REPLACE(t.id,'"', '')) as fdc_id
			, CONVERT(int, REPLACE(t.code,'"', ''))
			,t.description
		from #tmp t
		WHERE NOT EXISTS ( -- skip duplicates
			SELECT 1 FROM food_nutrient_source AS d -- UPDATE NAME
