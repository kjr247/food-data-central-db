...
BEGIN TRY  
	set @tableName = 'food_protein_conversion_factor';
	...
	TRUNCATE TABLE food_protein_conversion_factor;

	DROP TABLE IF EXISTS #tmp; 
	-- Mapping
	create table #tmp([food_nutrient_conversion_factor_id] VARCHAR(MAX) NOT NULL,	[value] VARCHAR(MAX) NULL)

	bulk insert #tmp
	From 'C:\Users\kjr24\Downloads\FoodData_Central_csv_2024-04-18\FoodData_Central_csv_2024-04-18\food_protein_conversion_factor.csv' -- UPDATE FILE name, (!sometimes the file names are different <crosses eyes>)
	WITH
		(
			CODEPAGE = '65001'
			,FIRSTROW = 2
	...
	-- DDL
	insert into food_protein_conversion_factor([food_nutrient_conversion_factor_id], [value]) -- UPDATE file name, and columns
		select 
			CONVERT(INT, REPLACE(t.food_nutrient_conversion_factor_id,'"', '')) as food_nutrient_conversion_factor_id
			,CAST(substring(t.value,2,len(t.value)-3) AS float) as value
		from #tmp t
		WHERE NOT EXISTS ( -- skip duplicates
			SELECT 1 FROM food_protein_conversion_factor AS d -- UPDATE NAME
			WHERE d.food_nutrient_conversion_factor_id = CONVERT(INT, REPLACE(t.food_nutrient_conversion_factor_id,'"', ''))
		)
