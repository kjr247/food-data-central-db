...
BEGIN TRY  
	set @tableName = 'nutrient_incoming_name';
	...
	TRUNCATE TABLE nutrient_incoming_name;

	DROP TABLE IF EXISTS #tmp; 
	-- Mapping
	create table #tmp(	id nvarchar(MAX) NOT NULL, [name] nvarchar(MAX) NULL, nutrient_id nvarchar(MAX) NULL)

	bulk insert #tmp
	From 'C:\Users\kjr24\Downloads\FoodData_Central_csv_2024-04-18\FoodData_Central_csv_2024-04-18\nutrient_incoming_name.csv' -- UPDATE FILE name, (!sometimes the file names are different <crosses eyes>)
	WITH
		(
			CODEPAGE = '65001'
			,FIRSTROW = 2
	...
	-- DDL
	insert into nutrient_incoming_name(id, name, nutrient_id) -- UPDATE file name, and columns
		select 
			CAST(REPLACE(t.id,'"', '') AS INT) as id
			, REPLACE(t.name, '"', '') AS name
			, CAST(substring(t.nutrient_id,2,len(t.nutrient_id)-3) AS INT) AS nutrient_id
		from #tmp t
		WHERE NOT EXISTS ( -- skip duplicates
			SELECT 1 FROM nutrient_incoming_name AS d -- UPDATE NAME
