...
BEGIN TRY  
	...
	-- Mapping
	create table #tmp(fdc_id nvarchar(max) NOT NULL, NDB_number nvarchar(max) NULL)

	bulk insert #tmp
	From 'C:\Users\kjr24\Downloads\FoodData_Central_csv_2024-04-18\FoodData_Central_csv_2024-04-18\sr_legacy_food.csv' -- UPDATE FILE name, (!sometimes the file names are different <crosses eyes>)
	WITH
		(
			CODEPAGE = '65001'
			,FIRSTROW = 2
	...
	-- DDL
	insert into sr_legacy_food(fdc_id, NDB_number) -- UPDATE file name, and columns
		select 
		...
		WHERE NOT EXISTS ( -- skip duplicates
			SELECT 1 FROM sr_legacy_food AS d -- UPDATE NAME
