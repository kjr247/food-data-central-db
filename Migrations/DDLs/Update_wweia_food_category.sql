...
BEGIN TRY  
	set @tableName = 'wweia_food_category';
	-- CHECKSUM
	...
	TRUNCATE TABLE wweia_food_category;

	DROP TABLE IF EXISTS #tmp; 
	-- Mapping
	create table #tmp([wweia_food_category] [nvarchar](max) NOT NULL,[wweia_food_category_description] [nvarchar](max) NULL)

	bulk insert #tmp
	From 'C:\Users\kjr24\Downloads\FoodData_Central_csv_2024-04-18\FoodData_Central_csv_2024-04-18\wweia_food_category.csv' -- UPDATE FILE name, (!sometimes the file names are different <crosses eyes>)
	WITH
		(
			CODEPAGE = '65001'
			,FIRSTROW = 2
	...
	-- DDL
	insert into wweia_food_category(wweia_food_category, wweia_food_category_description) -- UPDATE file name, and columns
		select 
			CONVERT(INT, REPLACE(t.wweia_food_category,'"', '')) as id
			, REPLACE(t.wweia_food_category_description,'"', '')
		from #tmp t
		WHERE NOT EXISTS ( -- skip duplicates
			SELECT 1 FROM wweia_food_category AS d -- UPDATE NAME
			WHERE d.wweia_food_category = CONVERT(INT, REPLACE(t.wweia_food_category,'"', ''))
		)
